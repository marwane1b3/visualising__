...
// const { appLocales, DEFAULT_LOCALE } = require('../../src/locales');
...
const FILES_TO_PARSE = 'src/**/messages.ts';
...
const toEvalCode = (code) => {
  try {
    let fileAsStr = code;

    fileAsStr = fileAsStr.replace('export const', 'const');
    fileAsStr = fileAsStr.replace('export default', '');
    return eval(fileAsStr);
  } catch (err) {
    console.error(err);
  }
};
...
const extractFromFile = async (filename) => {
  try {
    ...
    // const messages = get(output, '', []);

    const data = toEvalCode(code);
    const messages = getMessages(data);

    for (const message of messages) {
      for (const locale of appLocales) {
        const oldLocaleMapping = oldLocaleMappings[locale][message.scope];
        // Merge old translations into the babel extracted instances where react-intl is used
        const newMsg =
          locale === DEFAULT_LOCALE ? (message.options || {}).defaultValue : '';
        localeMappings[locale][message.scope] = oldLocaleMapping || newMsg;
      }
    }
  } catch (error) {
